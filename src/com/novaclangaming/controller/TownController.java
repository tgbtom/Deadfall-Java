...
public class TownController {
	...
	public String create(HttpServletRequest request, @RequestParam String townName, @RequestParam int townSize,
			@RequestParam int mapSize, @RequestParam String townMode) {
			...
			if (character.getCurrentAp() >= apToAssign && progress != null
				...
				if (transferAp) {
					...
					if (apToAssign >= structure.getApCost() - currentProgress.getAp()) {
						...
						request.getSession().setAttribute("message", "You have successfully levelled up "
								+ structure.getName() + " to level " + currentProgress.getLevel() + ".");
					} else {
						currentProgress.setAp(currentProgress.getAp() + apToAssign);
						request.getSession().setAttribute("message",
								"You have successfully assigned " + apToAssign + " AP to " + structure.getName() + ".");
					}
